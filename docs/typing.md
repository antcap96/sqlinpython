...
- **`sqlinpython.types`** — SQL type names (`Integer`, `Text`, `Varchar(255)`, `Decimal(10, 2)`, …) for column definitions and CAST — see [CREATE statements](./create.md).
- **`sqlinpython.typing`** — statement result types (`SelectStatement`, `InsertStatement`, …) and expression types (`Expression`, `ExpressionOrLiteral`, …) for annotating your own query-building helpers.
...
`sqlinpython.typing` exists so you can put types on functions that build or accept queries:

```python
from sqlinpython import Select, TableRef, col
from sqlinpython.typing import Expression, SelectStatement
